...
class ConfigIniParser implements ParserInterface
{
    ...
    protected function loadIniFile($ini_file)
    {
        if (!is_readable(realpath($ini_file))) {
            throw new Exception(sprintf('Unable to read config file at path: `%s`', $ini_file));
        }

        $settings = @parse_ini_file($ini_file, true);
        if ($settings === false) {
            throw new Exception(sprintf('Unable to parse given config file: `%s`', $ini_file));
        }
    ...
    protected function resolveDeployDirectory(array $settings, $config_dir)
    {
        if (!isset($settings['deploy_dir'])) {
            throw new Exception("Missing 'deploy_dir' setting within the provided config (.ini)file.");
        }
    ...
    protected function resolveCacheDirectory(array $settings, $config_dir)
    {
        if (!isset($settings['cache_dir'])) {
            throw new Exception("Missing 'cache_dir' setting within the provided config (.ini)file.");
        }
    ...
    protected function resolveTemplateDirectory(array $settings, $config_dir)
    {
        ...
        if (!is_dir($tpl_directory)) {
            var_dump($tpl_directory);
            throw new Exception("Unable to resolve given template_directory to an existing file system path.");
        }
    ...
    protected function resolveBootstrapFile(array $settings, $config_dir)
    {
        ...
        if (isset($settings['bootstrap_file'])) {
            ...
            if (!is_readable($bootstrap_file)) {
                throw new Exception(sprintf('Unable to read bootstrap file at path: `%s`', $bootstrap_file));
            }
        }
